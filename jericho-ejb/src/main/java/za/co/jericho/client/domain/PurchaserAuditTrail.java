 ...
 */
@Entity
@Table(name="purchaser_audit_trail", schema = "jericho")
public class PurchaserAuditTrail extends AbstractAuditTrailEntity {
    ...
    public void validate() {
        if (getPropertyFlipId() == null || getPropertyFlipId() < 1) {
            throw new EntityValidationException("PurchaserAuditTrail property flip id must be provided");
        }
        if (getContactId() == null || getContactId() < 1) {
            throw new EntityValidationException("PurchaserAuditTrail contact id must be provided");
        }
    }
